...
function parse(gltf, { fileName = 'model', ...options } = {}) {
  ...
  function handleProps(obj) {
    ...
    if (type === 'mesh' && node!=='nodes.Cube') {
      result += 
      `onPointerOver={(event) => props.handleMaterial({ meshName: '${obj.name}', type : 'hoverOver' })} 
      onPointerOut={(event) => props.handleMaterial({ meshName: '${obj.name}', type : 'hoverOut' })}`
      result += 
      ` onClick={(event) => props.handleMaterial({ meshName: '${obj.name}', type : 'clicked' })}`
    }
  ...
  const result = `${options.types ? `\nimport * as THREE from 'three'` : ''}
      ...
      : ''
        ...
        export ${options.exportdefault ? 'default' : ''} function Model(props${options.types ? ": JSX.IntrinsicElements['group']" : ''
          ...
          ${hasInstances ? 'const instances = useContext(context);' : ''} ${hasAnimations ? `const group = ${options.types ? 'useRef<THREE.Group>()' : 'useRef()'};` : ''
    } ${!options.instanceall
      ? `const { nodes, materials${hasAnimations ? ', animations' : ''} } = useGLTF('${url}'${options.draco ? `, ${JSON.stringify(options.draco)}` : ''
